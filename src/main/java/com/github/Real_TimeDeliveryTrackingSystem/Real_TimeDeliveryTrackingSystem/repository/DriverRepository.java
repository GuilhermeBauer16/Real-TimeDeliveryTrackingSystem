...
public interface DriverRepository extends JpaRepository<DriverEntity, String> {
    ...
    @Query("SELECT v FROM DriverEntity d JOIN d.vehicles v WHERE d.user.email = :driverEmail")
    Page<VehicleEntity> findVehiclesByDriverEmail(@Param("driverEmail") String driverEmail, Pageable pageable);
