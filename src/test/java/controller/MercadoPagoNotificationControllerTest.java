...
class MercadoPagoNotificationControllerTest extends AbstractionIntegrationTest {

    private static final String URL_PREFIX = "/ipn";
    ...
    private static final String EMAIL = "mercadopagonotification@example.com";
    ...
    void givenProductObject_whenCreateProduct_ShouldReturnProductObject() {
        ...
        var response = given().spec(specification)
                .contentType(TestConfigs.CONTENT_TYPE_JSON)
                .queryParam("id", TestConstants.ID)
                .queryParam("topic", PAYMENT_TOPIC)
                .filter(new RequestLoggingFilter(LogDetail.ALL))
                .filter(new ResponseLoggingFilter(LogDetail.ALL))
                .when()
                .post()
